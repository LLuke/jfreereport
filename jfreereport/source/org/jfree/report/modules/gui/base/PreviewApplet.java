 ...
 *
 * $Id: PreviewApplet.java,v 1.11 2005/02/23 21:04:48 taqua Exp $
 *
 * Changes
 * -------------------------
 ...
 * <p/>
 * Due to the special lifecycle of applets, this implementation must also provide methods
 * to handle the creation of reports. Implementations of this class must implement the
 * <code>getReport()</code> method to provide their fully initialized report definitions.
 * <p/>
 * The <code>getReport()</code> method is called from the <code>init()</code> method.
 *
...
public abstract class PreviewApplet extends JApplet implements PreviewProxy
{
   ...
   * <p/>
   * A subclass of <code>Applet</code> should override this method if it has any operation
   * that it wants to perform before it is destroyed. For example, an applet with threads
   * would use the <code>init</code> method to create the threads and the
   * <code>destroy</code> method to kill them.
   * <p/>
   * The implementation of this method provided by the <code>Applet</code> class does
   * nothing.
   *
   ...
   * initialization to perform. For example, an applet with threads would use the
   * <code>init</code> method to create the threads and the <code>destroy</code> method to
   * kill them.
   * <p/>
   * The implementation of this method provided by the <code>Applet</code> class does
   * nothing.
  ...
  /**
   * Returns the report that should be displayed in that applet. Make sure, that
   * subsequent calls to this method return the same report instance.
   * <p/>
   * This method is called from PreviewApplet.init(), so make sure that all initialization
   * is done before the init() method is called.
   *
