 ...
 * <p/>
 * The main method to call the dialog is {@link org.jfree.report.modules.gui.rtf.RTFExportDialog#performQueryForExport(JFreeReport)}.
 * Given a report, the dialog is shown and if the user approved the dialog, the RTF file
...
public class RTFExportDialog extends AbstractExportDialog
{
  ...
  private class ConfirmAction extends AbstractConfirmAction
  {
    ...
    {
      putValue(Action.NAME, resources.getString("rtf-exportdialog.confirm"));
    }
  }
  ...
  private class CancelAction extends AbstractCancelAction
  {
    ...
    {
      putValue(Action.NAME, resources.getString("rtf-exportdialog.cancel"));
    }
  }
  ...
  private class ActionSelectFile extends AbstractAction
  {
    ...
    {
      putValue(Action.NAME, resources.getString("rtf-exportdialog.selectFile"));
    }
  ...
  private void initialize ()
  {
    ...
    final JLabel lblFileName = new JLabel(getResources().getString("rtf-exportdialog.filename"));
    final JButton btnSelect = new ActionButton(getActionSelectFile());

    txFilename = new JTextField();
    cbStrictLayout = new JCheckBox(getResources().getString("rtf-exportdialog.strict-layout"));
  ...
  protected void performSelectFile ()
  {
    if (fileChooser == null)
    {
      ...
      final FilesystemFilter filter = new FilesystemFilter(".rtf", "RichTextFormat documents");
    ...
    if (option == JFileChooser.APPROVE_OPTION)
    {
      ...
      // Test if ends on xls
      if (StringUtil.endsWithIgnoreCase(selFileName, ".rtf") == false)
      {
        selFileName = selFileName + ".rtf";
      }
  ...
  public boolean performValidate ()
  {
    ...
    if (filename.trim().length() == 0)
    {
      getStatusBar().setStatus(JStatusBar.TYPE_ERROR,
              getResources().getString("rtf-exportdialog.targetIsEmpty"));
      return false;
    }
    ...
    if (f.exists())
    {
      ...
      {
        getStatusBar().setStatus(JStatusBar.TYPE_ERROR,
                getResources().getString("rtf-exportdialog.targetIsNoFile"));
        return false;
      }
      if (f.canWrite() == false)
      {
        getStatusBar().setStatus(JStatusBar.TYPE_ERROR,
                getResources().getString("rtf-exportdialog.targetIsNotWritable"));
        return false;
      }
