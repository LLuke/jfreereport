...
//	private PageFormat pageFormat;
   ...
   */
	...
	/** Sets the page number to be displayed.  What happens when the page number is out of range?-Nothing!
		...
		if (graphCache == null)
    {
      ...
      /** Paint Page Shadow */
			...
			Rectangle2D eastborder = new Rectangle2D.Double(outerW - 2 , outerY - 2 + PaperBorderPixel, PaperBorderPixel, outerH);
			g2.fill (eastborder);

			// Above Title
			Rectangle2D unprintArea = new Rectangle2D.Double (outerX, outerY, outerW - 2, (innerY - outerY));
			g2.setPaint(Color.white);
			g2.fill(unprintArea);

			// Under Footer
			unprintArea = new Rectangle2D.Double (outerX, innerY + innerH, outerW - 2, (outerH - innerY - innerH) - 2);
			g2.fill(unprintArea);
			...
			// Right
			unprintArea = new Rectangle2D.Double ((innerX + innerW), innerY, outerW - (innerX + innerW) - 2, (innerH));
			g2.fill(unprintArea);
    ...
    try
    {
      ...
      rpe.printStackTrace ();
		...
		if (pageIndex>pageCount-1) return Printable.NO_SUCH_PAGE;
