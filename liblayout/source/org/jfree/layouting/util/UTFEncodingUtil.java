...
public class UTFEncodingUtil
{
          ...
          throws UnsupportedEncodingException
  {
    final String encoding =
            LibLayoutBoot.getInstance().getGlobalConfig().getConfigProperty
            ("org.jfree.layouting.URLEncoding");
    ...
    for (int i = 0; i < chars.length; i++)
    {
      ...
      switch (ch)
      {
        case '%':
          ...
          final int hb = (Character.isDigit((char) lch)
                  ? lch - '0'
                  : 10 + Character.toLowerCase((char) lch) - 'a') & 0xF;
          final char hch = s.charAt(++i);
          final int lb = (Character.isDigit((char) hch)
                  ? hch - '0'
                  : 10 + Character.toLowerCase((char) hch) - 'a') & 0xF;
          b = (hb << 4) | lb;
          break;
        case '+':
          b = ' ';
